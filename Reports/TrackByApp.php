 ...
 *
 */
namespace Piwik\Plugins\OpeniAppTracker\Reports;
 ...
 */
class TrackByApp extends Base
{
    protected function init()
    {
        parent::init();

        $this->name          = Piwik::translate('OpeniAppTracker_TrackByApp');
        $this->dimension     = null;
...
//        $settings = new \Piwik\Plugins\OpeniAppTracker\Settings('OpeniAppTracker');
//        $columnToAggregate = $settings->realtimeReportDimension->getValue();
//        $columnLabel = \Piwik\Plugins\OpeniAppTracker\OpeniAppTracker::$availableDimensionsForAggregation[$columnToAggregate];


        $view->config->addTranslation('label', 'App');
    ...
    {
        $view = new View('@OpeniCompanyTracker/trackByCompany');
        $view->myData = array();
